...
[English](/TS-ANPR/index.md) | [한국어](/TS-ANPR/doc.i18n/ko-KR/index.md) | 日本語 | [Tiếng Việt](/TS-ANPR/doc.i18n/vi-VN/index.md)
...
#### v3.0.0 リリース (2025.5.27)🎉

1. 日本およびベトナムの車両ナンバープレート認識機能を追加

   - 日本の 140 地域、外交・自衛隊・旧式ナンバープレートに対応
   - ベトナムの自動車およびバイクのナンバープレートに対応
   - ライセンスに応じた国のサポート
     - 無料評価版ライセンス: アプリケーションの初期化関数で country 設定により国を指定 ([詳細](DevGuide.md#11-anpr_initialize))
     - 商用ライセンス: 初期化関数で指定した country は無視され、購入したライセンスに基づいて対応国が決まります。

2. 関心領域(RoI) / 非関心領域(RoU) 設定機能の追加
   - 入力画像に車番認識領域を設定できます。([詳細](DevGuide.md#23-関心領域-roi--関心領域-rou-の設定))
     ![](../../img/options/roi.png)
3. 最小ナンバープレートサイズ設定機能の追加

   - 最小サイズを設定すると、それより小さいナンバープレート領域は無視されます。 ([詳細](DevGuide.md#24-最小ナンバープレートサイズの設定))

4. 認識率の向上
   - アルゴリズム改善により、ナンバープレート認識精度が向上しました。
5. `tscam` モジュールの分離
   - `tscam`を `TS-ANPR` 配布から分離し、[TS-CAM Releases](https://github.com/bobhyun/TS-CAM/releases)で配布します。
   - 自己署名証明書を使用した HTTPS カメラのサポート
